...
app.get('/api/medications', authenticateToken, async (req, res) => {
  try {
    ...
    const Transacao = require('./models/Transacao');
    // Para cada medicamento, calcular retirados
    const medsWithRetirados = await Promise.all(meds.map(async (med) => {
      const retiradas = await Transacao.sum('quantidade', {
        where: {
      ...
      return {
        ...med.toJSON(),
        retirados: retiradas || 0
      };
    }));
    res.json({
      success: true,
      data: medsWithRetirados
    });
  } catch (error) {
...
app.post('/api/medications/:id/remover', authenticateToken, async (req, res) => {
  try {
    const id = parseInt(req.params.id);
    ...
    const quantidadeRemover = parseInt(quantidade);
    if (!quantidadeRemover || quantidadeRemover <= 0) {
      return res.status(400).json({
        success: false,
        message: 'Quantidade inválida para remoção'
      });
    }
    if (medicamento.quantidade < quantidadeRemover) {
      return res.status(400).json({
        success: false,
        message: 'Quantidade a remover maior que o estoque atual'
      });
    }
    medicamento.quantidade -= quantidadeRemover;
    if (medicamento.quantidade <= 0) {
      await medicamento.destroy();
      return res.json({
        success: true,
        message: 'Medicamento removido completamente do estoque'
      });
    } else {
      await medicamento.save();
      return res.json({
        success: true,
        message: 'Quantidade removida do medicamento',
        data: medicamento
      });
    }
  } catch (error) {
    console.error('Erro ao remover medicamento (POST):', error);
    res.status(500).json({
      success: false,
      message: 'Erro interno do servidor'
    });
...
(async () => {
  ...
  console.error('Erro ao inicializar dados ou iniciar servidor:', err);
  process.exit(1);
});
