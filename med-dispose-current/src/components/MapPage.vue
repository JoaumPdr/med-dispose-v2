<template>
  <div class="h-screen w-screen relative">
    ...
    <div id="map" class="w-full h-full bg-gray-200 flex items-center justify-center text-gray-500 text-lg">
      ...
      <div v-if="!mapLoaded && !loadingLocation && !locationError" class="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-red-100 border border-red-400 text-red-700 px-6 py-4 rounded shadow-lg z-50">
        <strong>Erro:</strong> O mapa não foi carregado corretamente.<br>
        Verifique sua conexão, a chave da API do Google Maps ou abra o console do navegador para mais detalhes.
      </div>
    </div>
  </div>
</template>
...
// Não buscar do backend, só usar os locais acima
...
const initMap = () => {
  ...
  try {
    ...
    // Adicionar marcadores para os locais reais
      ...
      const infoWindow = new window.google.maps.InfoWindow({
        content: `
          <div class="p-2">
            ...
            <p>Latitude: ${farmacia.latitude}</p>
            <p>Longitude: ${farmacia.longitude}</p>
          </div>
        `,
      });
...
const loadGoogleMapsScript = () => {
  ...
  if (document.querySelector('script[src*="maps.googleapis.com"]')) {
    ...
    // Se demorar mais de 5 segundos, mostrar erro
    setTimeout(() => {
      if (!window.google || !window.google.maps) {
        locationError.value = 'Erro ao carregar o Google Maps. Verifique sua conexão ou chave da API.';
        loadingLocation.value = false;
      }
    }, 5000);
    return;
  ...
  window.initGoogleMaps = () => {
    getUserLocation();
    // Se não carregar em 3s, mostrar erro
    setTimeout(() => {
      if (!mapLoaded.value) {
        locationError.value = 'O mapa não foi carregado corretamente. Verifique sua chave da API ou conexão.';
        loadingLocation.value = false;
      }
    }, 3000);
  };
